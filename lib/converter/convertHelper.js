...
const MultiPartFileResolver = require('./multiPartFileResolver');
const ZipFileResolver = require('./zipFileResolver');
...
class ConvertHelper {
  ...
  static convert(req, res, converter, convertFile, uuidResolver) {
    if (!req.query['zip-root-file'] && (!req.files || req.reqFiles.length === 1)) {
      ConvertHelper._convertStringToString(req, res, converter)
    } else {
      let rootFile = ''
      const uuid = uuidV4()
      if (req.query['zip-root-file']) {
        rootFile = req.query['zip-root-file']

...
class ConvertHelper {
  ...
  static _convertFromZipFile(req, res, converter, rootFile) {
    const tempDir = ConvertHelper._createTempDir().then((path) => {
    ...
    ConvertHelper._buildResponseMultipleFile(tempDir, res, converter, rootFile)
  }
  ...
  static _convertMultipleFiles(req, res, converter) {
    const tempDir = ConvertHelper._createTempDir().then((path) => {
    ...
    ConvertHelper._buildResponseMultipleFile(tempDir, res, converter, req.rootFile)
  }
  ...
  static convert(req, res, converter, convertFile) {
    if (req.query['zip-root-file']) {
      ConvertHelper._convertFromZipFile(req, res, convertFile, req.query['zip-root-file'])
    } else {
      if (!req.files || req.reqFiles.length === 1) {
        ConvertHelper._convertStringToString(req, res, converter)
      } else {
        ConvertHelper._convertMultipleFiles(req, res, convertFile)
      }
    }
  }
